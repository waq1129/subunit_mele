function [params, opt] = gen_params_rec_quad(opt)
...
for mm=1:opt.nModel
    ...
    w = alpha(mm)*(normpdf(1:opt.nDim_ws(mm),(opt.nDim_ws(mm)+1)/2,opt.nDim_ws(mm)/5)'-normpdf(1:opt.nDim_ws(mm),(opt.nDim_ws(mm)+1)/2,opt.nDim_ws(mm)/15)');
    ws = [ws w];
    ks = [ks k];
    a = -0.1;
    optmm.nDim = opt.nDim;
