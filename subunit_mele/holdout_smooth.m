function opt = holdout_smooth(x, y, opt)
% use a hold-out set to find optimal hyperparameters for smoothing
...
loglikelils = zeros(size(theta0,1),1);
...
for ii=1:size(theta0,1)
    ...
    loglikelils(ii) = floss_test_BSTC(C_ls, b_ls, a_ls);
end
% plot(loglikelils)
