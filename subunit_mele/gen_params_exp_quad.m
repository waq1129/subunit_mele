function [params, opt] = gen_params_exp_quad(opt)
...
Kbase = [Kbase; -normpdf(1:nk,(nk+1)/2,nk/3)+normpdf(1:nk,(nk+1)/2,nk/15)];
Kbase = Kbase';
for mm=1:opt.nModel
    ...
    k = normalize_x(k(:));
    opt.nDim_ws(mm) = opt.nDim0+1-opt.nDim_k0s(mm); % length of each w
    w = alpha(mm)*(normpdf(1:opt.nDim_ws(mm),(opt.nDim_ws(mm)+1)/2,opt.nDim_ws(mm)/5)'-normpdf(1:opt.nDim_ws(mm),(opt.nDim_ws(mm)+1)/2,opt.nDim_ws(mm)/15)'); % generate w
    ws = [ws w];
    ks = [ks k];
    a = -0.1;
    optmm.nDim = opt.nDim;
